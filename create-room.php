...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(!$username || !$room_name || !$room_password){
        $error = "All fields are required!";
    } else {
        // Set session for username
        $_SESSION['username'] = $username;

        // Generate 10-digit room ID
        $room_id = rand(1000000000,9999999999);

        // Create private_rooms table if not exists
        ...
        // Insert room
        $stmt = $conn->prepare("INSERT INTO private_rooms (room_id, room_name, room_password, created_by) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $room_id, $room_name, $room_password, $username);
        $stmt->execute();

        // Set session for stream
...
<title>Create Room - Secure Chat</title>
...
<div class="card">
    <h3 style="color:black;">Create Private Room</h3>
    <?php if(isset($error)){ echo "<div class='error'>$error</div>"; } ?>
    <form method="POST">
        <input  style="width:100%;border:.3px solid"  type="text" name="username" placeholder="Enter Username" required>
        <input  style="width:100%;border:.3px solid"  type="text" name="room_name" placeholder="Enter Room Name" required>
